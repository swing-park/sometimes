...
const MyPageToolBar = ({ isLogin }: Props) => {
  ...
  return (
    <StMyPageToolBar>
      ...
      {isLogin ? (
        <Button variant="outlined" color="error" size="small" onClick={handleOnClickLogOutBtn}>로그아웃</Button>
      ) : (
        <Button variant="outlined" size="small" onClick={() => window.location.replace("/signin")}>
          로그인
        </Button>
      )}
    </StMyPageToolBar>
